...
class MyData(data.Dataset):
    def __init__(self, root=None, label_txt=None,
        ...
        if root is None:
            self.root = "/navi/data/input_data/CUB_200_2011/"
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        return img, img, img, label, label, label
...
class CUB_200_2011:
    def __init__(self, width=224, origin_width=256, ratio=0.16, root=None, transform=None):
        ...
        if root is None:
            root = "/navi/data/input_data/CUB_200_2011/"

        train_txt = os.path.join(root, 'train.txt')
        test_txt = os.path.join(root, 'test.txt')

        self.train = MyData(root, label_txt=train_txt, transform=transform_Dict['rand-crop'], root = root)
        self.gallery = MyData(root, label_txt=test_txt, transform=transform_Dict['center-crop'], root = root)
